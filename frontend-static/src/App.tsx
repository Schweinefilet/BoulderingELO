...
const MarginalGainsContent = ({ counts }: { counts: Counts }) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: 4, backgroundColor: 'transparent', padding: 6, borderRadius: 6 }}>
    {ORDER.map((color: any) => (
...
function LoginScreen({ onLogin }: { onLogin: () => void }) {
  const [boulderingDone, setBoulderingDone] = useState(false);

  const handleBoulderingComplete = useCallback(() => {
    // Delay before ELO starts (in milliseconds) - tweak this value!
    setTimeout(() => setBoulderingDone(true), 0);
  }, []);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h1 className="mrs-saint-delafield-regular" style={{ marginTop: 0, marginBottom: 24, textAlign: 'center', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 4 }}>
          <SplitText
            text="Bouldering"
            className="italianno"
            delay={50}
            duration={1}
            ease="power3.out"
            onLetterAnimationComplete={handleBoulderingComplete}
          />
          <SplitText
            text="ELO"
...
export default function App() {
  ...
  const [backgroundType, setBackgroundType] = useState<'veil' | 'liquid' | 'pillars'>('veil');
  ...
  return (
    <>
      ...
      <div style={{ fontFamily: '"Red Hat Text", sans-serif', padding: '10px', maxWidth: 1000, margin: '0 auto', position: 'relative' }}>
        {backgroundEnabled && backgroundType === 'veil' && (
        ...
        {backgroundEnabled && backgroundType === 'pillars' && (
          <div style={{ position: 'fixed', inset: 0, zIndex: -1, pointerEvents: 'none' }}>
            <LightPillar
              topColor="#d3d3d3"
              bottomColor="#d3d3d3"
        ...
        {backgroundEnabled && backgroundType === 'liquid' && (
          <div style={{ position: 'fixed', inset: 0, zIndex: -1, pointerEvents: 'none' }}>
            <LiquidChrome
              baseColor={[0.1, 0.1, 0.15]}
              speed={isMobile ? 0.1 : 0.15}
              interactive={false}
              quality={isMobile ? 0.4 : 1.0}
            />
          </div>
        )}
        {imageViewer && (
                ...
                <div style={{ position: 'relative' }} ref={userMenuRef}>
                  ...
                  {userMenuOpen && (
                    ...
                    >
                      ...
                      {backgroundEnabled && (
                        <div style={{ padding: '4px 14px 10px 14px', display: 'flex', gap: 6 }}>
                          <button
                            onClick={() => setBackgroundType('veil')}
                            style={{
                              flex: 1,
                              fontSize: 11,
                              padding: '4px 0',
                              backgroundColor: backgroundType === 'veil' ? '#3b82f6' : 'rgba(255,255,255,0.05)',
                              color: backgroundType === 'veil' ? 'white' : '#94a3b8',
                          ...
                          <button
                            ...
                            style={{
                              ...
                              fontSize: 11,
                              padding: '4px 0',
                              backgroundColor: backgroundType === 'liquid' ? '#3b82f6' : 'rgba(255,255,255,0.05)',
                              color: backgroundType === 'liquid' ? 'white' : '#94a3b8',
                              border: '1px solid rgba(255,255,255,0.1)',
                              borderRadius: 4,
                              cursor: 'pointer'
                            }}
                          ...
                          <button
                            onClick={() => setBackgroundType('pillars')}
                            style={{
                              flex: 1,
                              fontSize: 11,
                              padding: '4px 0',
                              backgroundColor: backgroundType === 'pillars' ? '#3b82f6' : 'rgba(255,255,255,0.05)',
                              color: backgroundType === 'pillars' ? 'white' : '#94a3b8',
                              border: '1px solid rgba(255,255,255,0.1)',
                              borderRadius: 4,
                              cursor: 'pointer'
                            }}
        ...
        <div style={{ position: 'relative', marginBottom: 20 }}>
          ...
          <div style={{ backgroundColor: 'transparent', padding: 24, borderRadius: 8, border: BLACK_PANEL_BORDER, position: 'relative' }}>
                                  ...
                                  <div>
                                    ...
                                    {positionEditMode && (
                                      <div style={{ display: 'flex', gap: 8, marginTop: 4, marginBottom: 12, flexWrap: 'wrap' }}>
                                        <button
                                          onClick={async () => {
                                            const positionedRoutes = routesForWall.filter(r => getRoutePositionForImage(r, safeIndex));
                                            if (positionedRoutes.length === 0) {
                                              setToast({ message: "No markers to clear on this image", type: "error" });
                                              setTimeout(() => setToast(null), 2000);
                                              return;
                                            }
                                            if (confirm(`Are you sure you want to clear ALL ${positionedRoutes.length} markers on this image?`)) {
                                              setLoading(true);
                                              try {
                                                for (const r of positionedRoutes) {
                                                  await clearRoutePosition(r, safeIndex);
                                                }
                                                setToast({ message: `Cleared ${positionedRoutes.length} markers`, type: 'success' });
                                              } finally {
                                                setLoading(false);
                                                setTimeout(() => setToast(null), 2000);
                                              }
                                        ...
                                        <button
                                          ...
                                          style={{
                                            padding: '6px 12px',
                                            backgroundColor: autoAdvance ? '#3b82f6' : '#475569',
                                            color: 'white',
                                            border: 'none',
                                            borderRadius: 6,
                                            cursor: 'pointer',
                                        ...
                                        >
                                          {autoAdvance ? '🔄 Auto-Advance: ON' : '⏹️ Auto-Advance: OFF'}
                                        </button>
                                      </div>
                                    )}
                              ...
                              <div style={{ fontSize: 14, fontWeight: '600', marginBottom: 12, color: '#94a3b8' }}>
                                {positionEditMode ? (() => {
                                  ...
                                  return (
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                      <div>
                                        <div style={{ color: nextRoute ? '#fff' : '#94a3b8' }}>{nextRoute
                                        ...
                                        <div style={{ fontSize: 11, color: '#64748b', marginTop: 4 }}>
                                          Click the (✕) handle or right-click to delete a marker
                                        </div>
                                      </div>
                                      {nextRoute && (
                                        <button
                                          onClick={() => setRouteToPosition(null)}
                                          style={{
                                            padding: '6px 12px',
                                            backgroundColor: '#475569',
                                            color: 'white',
                                  ...
                                  return (
                                    <div
                                      ...
                                      style={{
                                        ...
                                        zIndex: isActiveOverlay ? 20 : (isPendingPosition ? 15 : 5),
                                        transition: 'all 0.2s'
                                      }}
                                    >
                                      <button
                                        onClick={(e) => {
                                          ...
                                          if (positionEditMode && user?.role === 'admin') {
                                            ...
                                            if (isSelected) {
                                              setSelectedRoutes(prev => prev.filter(id => id !== route.id));
                                              if (overlayRouteId === route.id) setOverlayRouteId(null);
                                            } else {
                                              setSelectedRoutes(prev => [...prev, route.id!]);
                                      ...
                                      {positionEditMode && user?.role === 'admin' && (
                                        <button
                                          onClick={(e) => {
                                            e.preventDefault();
                                            e.stopPropagation();
                                            if (confirm(`Delete position for Route #${route.section_number}?`)) {
                                              clearRoutePosition(route, safeIndex);
                                            }
                                          }}
                                          style={{
                    ...
                    <div style={{ backgroundColor: BLACK_PANEL_BG, padding: 'clamp(12px, 3vw, 16px)', borderRadius: 8, fontSize: 13, border: BLACK_PANEL_BORDER, overflowX: 'auto' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, flexWrap: 'wrap', justifyContent: 'space-between' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                          ...
                          >
                            <div style={{ color: '#a5b4fc', fontWeight: 800, fontSize: isMobileCompact ? 14 : 16, display: 'flex', alignItems: 'center', gap: 2 }}>
                              <span>Score:</span>
                              <Counter
                                value={previewScore}
                                fontSize={isMobileCompact ? 14 : 16}
                                places={[100, 10, 1, 0.1, 0.01]}
                                textColor="#a5b4fc"
                                fontWeight={800}
                                gradientHeight={0}
          ...
          <div style={{ padding: 24, backgroundColor: BLACK_PANEL_BG, borderRadius: PANEL_RADIUS, border: BLACK_PANEL_BORDER }}>
            ...
            <div style={{
              ...
              {(showAllLeaderboard ? leaderboard : leaderboard.slice(0, 10)).map((e: any, i: number) => {
                ...
                const defaultRowColor = 'transparent';
                return (
                  <div
                    key={i}
                    style={{
                    ...
                    onMouseEnter={e => {
                      ...
                      e.currentTarget.style.setProperty('--row-bg-color', 'rgba(255, 255, 255, 0.05)');
                    }}
                    onMouseLeave={e => {
                ...
                <div style={{
                  ...
                  <button
                    ...
                    style={{
                      ...
                      backgroundColor: BLACK_ROW_BG,
                      color: '#bfdbfe',
                      border: `1px solid ${BLACK_PANEL_BORDER_COLOR}`,
                      borderRadius: 999,
                      fontSize: 14,
                      fontWeight: '600',
                      cursor: 'pointer',
                    ...
                    onMouseLeave={e => {
                      e.currentTarget.style.backgroundColor = BLACK_ROW_BG;
                      e.currentTarget.style.borderColor = BLACK_PANEL_BORDER_COLOR;
                    }}
                  >
        ...
        <section style={{ marginTop: 32 }}>
          ...
          <div style={{ padding: 24, backgroundColor: 'transparent', borderRadius: PANEL_RADIUS, border: BLACK_PANEL_BORDER }}>
        ...
        <section id="analytics" style={{ marginTop: 24 }}>
          ...
          <div style={{ marginTop: 24, padding: 24, borderRadius: PANEL_RADIUS, border: BLACK_PANEL_BORDER }}>
            <h3 style={{ marginTop: 0 }}>Total Score Over Time - Top 10 (Last 30 Days)</h3>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart
                data={(() => {
                  ...
                  // Track each climber's current total score
                  // Initialize with the score from the latest session BEFORE the window, if any
                  const climberCurrentScore: { [climberId: number]: number } = {};

                  climbers.forEach((c: any) => {
                    const priorSession = sessions
                      .filter((s: any) => s.climberId === c.id && new Date(s.date) < startDate)
                      .sort((a: any, b: any) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];

                    climberCurrentScore[c.id] = priorSession ? priorSession.score : 0;
                  });

                  // Sessions within the window
                  const sessionsInWindow = sessions
                    .filter((s: any) => new Date(s.date) >= startDate)
                    .sort((a: any, b: any) => new Date(a.date).getTime() - new Date(b.date).getTime());

                  const chartData: any[] = [];
                  allDates.forEach((date: string) => {
                    // Update scores from sessions on this date
                    sessionsInWindow.filter((s: any) => s.date === date).forEach((s: any) => {
                    ...
                    climbers.forEach((c: any) => {
                      // Only show line if they have a non-zero score or had a prior session
                      const score = climberCurrentScore[c.id];
                      point[c.name] = score > 0 ? score : null;
                    });
        ...
        {viewingProfile !== null && (() => {
          ...
          return (
            <div
              ...
              >
                {/* Header Section */}
                <div style={{ borderTopLeftRadius: 10, borderTopRightRadius: 10 }}>
                  <div style={{
                    backgroundColor: '#000',
                    ...
                    <div style={{ display: 'flex', gap: 24, alignItems: 'flex-start', flexWrap: 'wrap' }}>
                      <div style={{ flex: 1, minWidth: 200 }}>
                        ...
                        {(profileClimber?.bio || profileClimber?.started_bouldering || profileClimber?.instagram_handle) && (
                          <div style={{ marginTop: 16, padding: 16, backgroundColor: 'transparent', borderRadius: 8, border: '1px solid #fff' }}>
