...
export const Tooltip = ({
  ...
  const calculatePosition = (mouseX: number, mouseY: number) => {
    ...
    // If right overflow still occurs, clamp within viewport
    if (absoluteX + finalX + tooltipWidth > viewportWidth) {
      finalX = Math.max(-containerRect.left + 12, viewportWidth - containerRect.left - tooltipWidth - 12);
    }
      ...
      <AnimatePresence>
        {isVisible && (
          <motion.div
            ...
            className="pointer-events-none absolute z-50 min-w-[10rem] overflow-hidden rounded-md border border-transparent bg-white shadow-sm ring-1 shadow-black/5 ring-black/5 dark:bg-neutral-900 dark:shadow-white/10 dark:ring-white/5"
            style={{
              top: position.y,
              left: position.x,
              maxHeight: "12rem"
          ...
          >
            <div
              ref={contentRef}
              className="p-2 text-sm text-neutral-600 dark:text-neutral-400"
            >
              {content}
            </div>
          </motion.div>
